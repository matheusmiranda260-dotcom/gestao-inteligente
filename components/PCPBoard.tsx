        ...
        <div className={`pcp-board-container min-h-screen bg-[#08131B] text-slate-100 flex flex-col gap-3 select-none ${
            ...
            <div className={`flex-1 pcp-glass rounded-2xl border border-white/5 shadow-2xl flex flex-col overflow-hidden ${
                ...
                <div className="bg-[#0A1B27]/80 backdrop-blur-md border border-white/10 p-3.5 rounded-2xl shadow-xl flex flex-col gap-3.5 transition-all animate-fade">
                    ...
                    {/* Linha 2: Monitor de Máquinas & Paradas em Tempo Real (Chão de Fábrica ao Vivo) */}
                        ...
                        <div className="flex items-center justify-between px-1">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] font-black text-white uppercase tracking-wider flex items-center gap-1.5">
                                    ...
                                    <span className="text-[9px] text-[#00E5FF]/80 font-mono font-normal">• 1s Live Feed</span>
                                </span>
                            </div>
                            <div className="hidden md:flex items-center gap-3 text-[9px] font-bold uppercase tracking-wider">
                                <span className="flex items-center gap-1 text-[#00E5FF]">
                                    <span className="w-1.5 h-1.5 rounded-full bg-[#00E5FF] pulse-live" /> Operando
                                </span>
                                <span className="flex items-center gap-1 text-amber-400">
                                    <span className="w-1.5 h-1.5 rounded-full bg-amber-400" /> Setup/Prep
                                </span>
                                <span className="flex items-center gap-1 text-rose-400">
                                    <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" /> Parada
                                </span>
                                <span className="flex items-center gap-1 text-slate-400">
                                    <span className="w-1.5 h-1.5 rounded-full bg-slate-500" /> Disponível
                                </span>
                            </div>
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2.5">
                            {machineLiveStatus.map((m) => {
                                const isStop = m.state === 'stopped';
                                const isPrep = m.state === 'prep';
                                const isProd = m.state === 'producing';
                                const isOffline = m.state === 'offline';

                                let cardBg = 'bg-[#08151F]/90 border-white/10 hover:border-white/20';
                                if (isStop) {
                                    cardBg = 'bg-[#220B10]/95 border-rose-500/60 shadow-[0_0_20px_rgba(244,63,94,0.25)] hover:border-rose-500';
                                } else if (isPrep) {
                                    cardBg = 'bg-[#1E1406]/95 border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.15)] hover:border-amber-400';
                                } else if (isProd) {
                                    cardBg = 'bg-[#06202C]/95 border-[#00E5FF]/40 shadow-[0_0_15px_rgba(0,229,255,0.12)] hover:border-[#00E5FF]/80';
                                } else if (isOffline) {
                                    cardBg = 'bg-[#0F172A]/90 border-slate-500/50 hover:border-slate-400';
                                }
                                ...
                                return (
                                    <div 
                                        key={m.machine}
                                        onClick={() => {
                                            localStorage.setItem('msm_active_machine', m.machine);
                                            if (m.machine.startsWith('Trefila')) setPage(isProd || isStop || isPrep || isOffline ? 'trefilaInProgress' : 'trefilaPending');
                                            else if (m.machine.startsWith('Treliça')) setPage(isProd || isStop || isPrep || isOffline ? 'trelicaInProgress' : 'trelicaPending');
                                            else if (m.machine.startsWith('Malha')) setPage(isProd || isStop || isPrep || isOffline ? 'malhaInProgress' : 'malhaPending');
                                        }}
                                        className={`p-3 rounded-xl border transition-all cursor-pointer flex flex-col justify-between min-h-[96px] relative group overflow-hidden ${cardBg}`}
                                        title={`Clique para ir ao painel da máquina (${m.machine})`}
                                    >
                                        <div className="flex items-start justify-between gap-1">
                                            <div className="flex flex-col">
                                                <div className="flex items-center gap-1.5">
                                                    <span className={`w-2 h-2 rounded-full ${m.dotColor}`} />
                                                    <span className="text-xs font-black text-white tracking-wide">{m.machine}</span>
                                                </div>
                                                <span className="text-[9px] text-slate-400 font-bold uppercase mt-0.5">{m.type}</span>
                                            </div>

                                            <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-md border tracking-wider whitespace-nowrap ${m.badgeColor}`}>
                                                {m.badgeText}
                                            </span>
                                        </div>

                                        <div className="mt-2 pt-2 border-t border-white/5 flex items-end justify-between">
                                            {isOffline ? (
                                                <div className="flex flex-col">
                                                    <span className="text-[9px] font-black uppercase text-slate-300 truncate max-w-[130px]" title={m.reason}>
                                                        {m.reason}
                                                    </span>
                                                </div>
                                            ) : isStop ? (
                                                <div className="flex flex-col">
                                                    <span className="text-[9px] font-black uppercase text-rose-300 truncate max-w-[130px]" title={m.reason}>
                                                        {m.reason}
                                                    </span>
                                                    <span className="text-sm font-black font-mono text-rose-400 leading-none mt-1 animate-pulse">
                                                        ⏱️ {formatDuration(m.durationMs)}
                                                    </span>
                                                </div>
                                            ) : isPrep ? (
                                                <div className="flex flex-col">
                                                    <span className="text-[9px] font-black uppercase text-amber-300 truncate max-w-[130px]" title={m.reason}>
                                                        {m.reason}
                                                    </span>
                                                    <span className="text-sm font-black font-mono text-amber-400 leading-none mt-1">
                                                        ⏱️ {formatDuration(m.durationMs)}
                                                    </span>
                                                </div>
                                            ) : isProd ? (
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="text-[10px] font-black text-[#00E5FF] leading-none mt-0.5">
                                                        {m.type === 'Trefila' ? `Bitola ${m.op?.targetBitola}mm` : m.type === 'Treliça' ? m.op?.trelicaModel : m.op?.malhaModel || 'Produzindo'}
                                                    </span>
                                                </div>
                                            ) : (
                                                <div className="flex flex-col">
                                                    <span className="text-[9px] font-medium text-slate-400 truncate max-w-[130px]">
                                                        {m.reason}
                                                    </span>
                                                </div>
                                            )}

                                            <span className="text-[9px] text-slate-400 group-hover:text-[#00E5FF] font-bold transition-colors whitespace-nowrap">
                                                Painel →
                                            </span>
                                        </div>
                                    </div>
