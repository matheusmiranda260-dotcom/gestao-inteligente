...
// Interfaces locais para estruturação do Relatório da Malha
interface StopRow {
    id: string;
...
const ReportsMalha: React.FC<ReportsMalhaProps> = ({ stock, setPage }) => {
    // 1. Estados de Controle da Página
    const [selectedMachine, setSelectedMachine] = useState<'Malha 1' | 'Malha 2'>('Malha 1');
    ...
    const loadDraftForMachine = (machine: 'Malha 1' | 'Malha 2') => {
        setLoading(true);
        const localKey = `malha_report_draft_${machine}`;
        ...
        if (draft) {
            try {
                ...
                setProductDescription(parsed.product_description || 'MALHA H-12 LEVE 6 MTS');
    ...
    const resetFormToDefault = () => {
        ...
        setOperatorShiftA('');
        setOperatorShiftB('');
        setProductDescription('MALHA H-12 LEVE 6 MTS');
    ...
    const saveCurrentState = (machine: 'Malha 1' | 'Malha 2', date: string, options?: { showToastAlert?: boolean }) => {
    ...
    useEffect(() => {
        ...
        autoSaveTimerRef.current = setTimeout(() => {
            const localKey = `malha_report_draft_${selectedMachine}`;
            const reportData = {
    ...
    const getSavedDatesKey = (machine: 'Malha 1' | 'Malha 2') =>
        `malha_saved_dates_${machine}`;

    // Obtém o conjunto de datas já salvas para uma máquina
    const getSavedDates = (machine: 'Malha 1' | 'Malha 2'): string[] => {
        try {
    ...
    const registerSavedDate = (machine: 'Malha 1' | 'Malha 2', date: string) => {
    ...
    const handleSwitchMachine = (newMachine: 'Malha 1' | 'Malha 2') => {
    ...
    const copyToClipboard = async () => {
        try {
            const element = document.getElementById('malha-report-sheet');
            if (!element) return;
            ...
            const canvas = await html2canvas(element, {
                ...
                onclone: (clonedDoc) => {
                    ...
                    const clonedElement = clonedDoc.getElementById('malha-report-sheet');
            ...
            canvas.toBlob(async (blob) => {
                if (blob) {
                    try {
                        ...
                        link.download = `Relatorio_malha_${selectedDate}.png`;
        ...
        <div className="p-4 sm:p-6 md:p-8 bg-slate-50 min-h-screen font-mono text-slate-800 relative select-none">
            ...
            <style dangerouslySetInnerHTML={{ __html: `
                ...
                @media print {
                    ...
                    * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
                    ...
                    #malha-report-sheet, #malha-report-sheet * {
                        overflow: visible !important; max-height: none !important;
                    }
                    #malha-report-sheet { height: auto !important; }
                    #malha-report-sheet img {
                        max-height: 72px !important; height: auto !important;
                        object-fit: contain !important; display: block !important;
                    }
                    #malha-report-sheet table, #malha-report-sheet thead,
                    #malha-report-sheet tbody, #malha-report-sheet tr,
                    #malha-report-sheet td, #malha-report-sheet th {
                        page-break-inside: avoid !important; break-inside: avoid !important;
                        overflow: visible !important;
                    }
                    #malha-report-sheet thead { display: table-header-group !important; }
                    #malha-report-sheet tbody { display: table-row-group !important; }
                    .modern-editable-input {
                            ...
                            <div className="w-full lg:w-1/2 flex flex-col justify-between space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <div className="w-full bg-slate-200 border border-slate-300 text-slate-600 font-bold rounded-lg px-3 py-2 cursor-not-allowed">
                                            {franjaTransversal.toFixed(2)}
                                        </div>
                                    </div>
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-black text-slate-500 uppercase tracking-widest">Franja Longitudinal (cm)</label>
                                        <div className="w-full bg-slate-200 border border-slate-300 text-slate-600 font-bold rounded-lg px-3 py-2 cursor-not-allowed">
                                            {franjaLongitudinal.toFixed(2)}
                                        </div>
                                    </div>
                                </div>
            ...
            <section className="bg-white p-4 rounded border border-slate-200 shadow-sm mb-4 flex flex-col sm:flex-row items-center justify-between gap-4 no-print">
                <div className="flex bg-slate-100 p-1 rounded border border-slate-200 w-full sm:w-auto">
                    {(['Malha 1', 'Malha 2'] as const).map(machine => (
                        <button
                            key={machine}
                        ...
                        <div className="border border-[#002060] rounded-lg overflow-hidden bg-white shadow-sm">
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-between border-b border-slate-200 bg-slate-50/50 py-2 px-4 gap-2">
                                <div className="flex items-center gap-1 text-xs font-bold text-slate-700">
                                    ...
                                    />
                                    <span className="text-slate-500 font-medium">malhas</span>
                                </div>
